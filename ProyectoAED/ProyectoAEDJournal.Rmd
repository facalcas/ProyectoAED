...
suicidios_por_sexo<-datos_ns_tidy%>%
  group_by(Año,Sexo)%>%
  summarise(Total_Suicidios=sum(Total))
...
gr_por_sexo<-ggplot(suicidios_por_sexo,aes(x=Año, y=Total_Suicidios, fill=Sexo)) +
  geom_col(position = "dodge") +
  labs(title = "Suicidios en España por Sexo en el periodo 2014-2023",
       x="Año",
       y="Número de Suicidios")+
  scale_fill_manual(values = c("Hombres" = "skyblue", "Mujeres" = "pink")) +
  theme_minimal()

ggplotly(gr_por_sexo)

gr_li_por_nacionalidad<-ggplot(suicidios_por_nacionalidad,aes(x=Año,y=Total_Suicidios,colour=Nacionalidad,group=Nacionalidad))+
  geom_line()+
  geom_point()+
  labs(title="Suicidios en España por Nacionalidad en el periodo 2024-2023",
       x="Año",
       y="Número de Suicidios")+
  theme_minimal()

ggplotly(gr_li_por_nacionalidad)

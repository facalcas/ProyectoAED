...
```{r}
pacman::p_load(tidyverse,lubridate,dplyr,stringr,readr)
```
...
CSE<-grep("CSE\\.csv$", archivos, value = TRUE)
...
df <- df %>%
  mutate(
    Territorio = Territorio %>%
      ...
      str_squish() %>%
      # 2) Quitar prefijo numérico + espacio (p.ej. "03 "):
      str_remove("^\\d+\\s+") %>%
      # 3) Quedarse con lo anterior a la primera coma (p.ej. "Asturias, Principado de" -> "Asturias")
      str_remove(",.*$") %>%
